...
class FlaisResourceReconcilerTest : KoinTest {
    ...
    @Test
    fun `should not set correlation id on FlaisResource if exists`(context: KubernetesOperatorContext) {
        val testResource =
    ...
    @Test
    fun `should set correct synchronization hash`(context: KubernetesOperatorContext) {
        val testResource = context.createAndGetTestResource(createTestResource())

        assertNotNull(testResource)
        assertNotNull(testResource.status.synchronizationHash)
        assertEquals(testResource.resourceHash(), testResource.status.synchronizationHash)
    }
    ...
    fun `should handle dependent errors`(context: KubernetesOperatorContext) {
        ...
        assertEquals(
            "error",
            actualTestResource.status.errors
                ?.find { it.dependent == service.name() }
                ?.message,
        )
    }
