...
class FlaisResourceReconciliationFilterTest : KoinTest {
    ...
    @Test
    fun `should reconcile when correlation id changes on startup`(context: KubernetesOperatorContext) {
        val resource = context.createAndGetTestResource(createTestResource())
        assertNotNull(resource)
        val logsCaptor = captureReconcilerLogs()
    ...
    companion object {
        ...
        private fun captureOperatorLogs() = LogCaptor.forName("io.javaoperatorsdk").apply { setLogLevelToDebug() }
