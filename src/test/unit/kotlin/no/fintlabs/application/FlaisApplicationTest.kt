...
class FlaisApplicationTest {
    ...
    fun beforeEach() {
        CustomResourceDefinitionContext
            .v1CRDFromCustomResourceType(FlaisApplication::class.java)
            .build()
            .let {
                crd = it
                client
    ...
    fun `FlaisApplicationCrd should have correct onePassword`() {
        createAndApplyFlaisApplication(FlaisApplicationSpec(onePassword = OnePassword("test-itemPath")))
        val resFlaisApplication = getFlaisApplication()
    ...
    fun `FlaisApplicationCrd should have correct kafka`() {
        ...
        assertEquals(
            "test-resource",
            resFlaisApplication.spec.kafka.acls
                .first()
                .topic,
        )
        assertEquals(
    ...
    fun `FlaisApplicationCrd should have correct database`() {
        createAndApplyFlaisApplication(FlaisApplicationSpec(database = Database("test-database")))
        val resFlaisApplication = getFlaisApplication()
    ...
    fun `FlaisApplicationCrd should have correct legacy ingress`() {
        ...
        assertEquals(
            1,
            resFlaisApplication.spec.ingress
                ?.middlewares
                ?.size,
        )
        assertEquals(
