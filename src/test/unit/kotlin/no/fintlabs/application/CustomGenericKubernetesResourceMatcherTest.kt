...
class CustomGenericKubernetesResourceMatcherTest {
  @MockK lateinit var context: DefaultContext<HasMetadata>
  ...
  fun `test deployment with different resource value representation`() {
    ...
    val desired =
        loadResource<Deployment>("/resource-matcher/kubernetes/simple-deployment-desired.yaml")
  ...
  fun `test replica set with different resource value representation`() {
    ...
    val desired =
        loadResource<ReplicaSet>("/resource-matcher/kubernetes/simple-replica-set-desired.yaml")
    val match = matcher.matches(actual, desired, context)
