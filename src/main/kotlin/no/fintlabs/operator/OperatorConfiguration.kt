...
class OperatorConfiguration : BaseConfigurationService(), OperatorConfigurationOverrider, KoinComponent {
    ...
    override fun getKubernetesClient(): KubernetesClient
        = kubernetesClient ?: get()

    override fun closeClientOnStop(): Boolean
        = closeClientOnStop ?: super.closeClientOnStop()

    override fun getMetrics(): Metrics =
        metrics ?: get()

    override fun dependentResourceFactory(): DependentResourceFactory<out ControllerConfiguration<*>, out DependentResourceSpec<*, *, *>> {
    ...
    override fun <P : HasMetadata> configFor(reconciler: Reconciler<P>): ControllerConfiguration<P> {
        ...
        if (workflowAnnotation != null) {
            ...
            config.setWorkflowSpec(object : WorkflowSpec {
    ...
    private fun dependentResources(workflow: Workflow, controllerConfiguration: ControllerConfiguration<*>): List<DependentResourceSpec<*, *, *>> {
        val dependents = workflow.dependents
        return dependents.map { dependent ->
            val qualifier = dependent.qualifier.takeIf { it.isNotEmpty() }?.let { named(it) }
            val dependentInstance = getKoin().get<DependentResource<Any, HasMetadata>>(dependent.dependentClass, qualifier)
            val dependentType = dependentInstance.javaClass

            val readyCondition = if (dependentInstance is ReadyCondition<*>)  {
                KCondition { _, p, c -> (dependentInstance as ReadyCondition<HasMetadata>).isReady(p, c) }
            } else null

            val reconcileCondition = if (dependentInstance is ReconcileCondition<*>)  {
                KCondition { _, p, c -> (dependentInstance as ReconcileCondition<HasMetadata>).shouldReconcile(p, c) }
            } else null
            ...
            KoinDependentResourceSpec(
                ...
                DependentResourceConfigurationResolver
                    .configureSpecFromConfigured(it, controllerConfiguration, dependentType)
            }
        }.toList()
    }
}