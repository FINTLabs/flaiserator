...
class JobDR :
    ...
    KoinComponent,
    Creator<Job, FlaisJob>,
    ReconcileCondition<FlaisJob>,
    ReadyCondition<FlaisJob> {
    ...
    return Job().apply {
      metadata =
          ObjectMeta().apply {
            ...
            annotations = mutableMapOf("cronjob.kubernetes.io/instantiate" to "manual")
            ownerReferences =
                mutableListOf(
                    createOwnerReference(cronJob).apply {
                      controller = true
  ...
  override fun shouldReconcile(primary: FlaisJob, context: Context<FlaisJob>): Boolean =
      primary.spec.schedule.isNullOrEmpty()

  override fun isReady(primary: FlaisJob, context: Context<FlaisJob>): Boolean {
