...
data class Ingress(
    @Deprecated("Not used in future versions. Use routes instead") val enabled: Boolean = false,
    @Deprecated("Not used in future versions. Use routes instead")
    @ValidationRule("self.matches('^/[A-Za-z0-9/_-]*$')", message = "Invalid path")
  ...
  data class Route(
    ...
    @JsonIgnore lateinit var allMiddlewares: Set<String>
  }
}

fun Ingress.isLegacy(): Boolean = this.routes.isNullOrEmpty()

fun FlaisApplicationSpec.isIngressEnabled() =
    when {
      ingress == null -> false
