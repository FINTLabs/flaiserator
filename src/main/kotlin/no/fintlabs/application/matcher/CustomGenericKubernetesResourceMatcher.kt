...
class CustomGenericKubernetesResourceMatcher<R : HasMetadata> {
    companion object {
        ...
        private val IGNORED_METADATA =
            listOf("creationTimestamp", "deletionTimestamp", "generation", "selfLink", "uid")

        @Suppress("UNCHECKED_CAST")
        fun <L : HasMetadata> getInstance(): CustomGenericKubernetesResourceMatcher<L> =
            INSTANCE as CustomGenericKubernetesResourceMatcher<L>
    ...
    fun matches(
        ...
        val optionalManagedFieldsEntry =
            checkIfFieldManagerExists(actual, context.controllerConfiguration.fieldManager())
        ...
        val actualMap =
            objectMapper.convertValue(actual, MutableMap::class.java) as MutableMap<String, Any>
        val desiredMap =
            objectMapper.convertValue(desired, MutableMap::class.java) as MutableMap<String, Any>
    ...
    private fun sanitizeState(
        actual: R,
        desired: R,
        actualMap: MutableMap<String, Any>,
    ) {
        if (desired is StatefulSet && actual is StatefulSet) {
            ...
            if (claims == actualStatefulSet.spec.volumeClaimTemplates.size) {
                for (i in 0 until claims) {
                    if (desiredStatefulSet.spec.volumeClaimTemplates[i]
                            .spec
                            ?.volumeMode == null
                    ) {
                        GenericKubernetesResource
                            .get<MutableMap<String, Any>?>(
                                actualMap,
                                "spec",
    ...
    private fun normalizeValuesForComparison(
        resource: R,
    ...
    private fun handleListKeyEntrySet(
        ...
        for ((listEntryKey, listEntryValue) in managedEntrySet) {
            ...
            val actualListEntry =
                selectListEntryBasedOnKey(keyWithoutPrefix(listEntryKey), actualValueList, objectMapper)
    ...
    private fun checkIfFieldManagerExists(
        ...
        val targetManagedFields =
            actual.metadata.managedFields
                ?.filter { it.manager == fieldManager && it.operation == APPLY_OPERATION }
                .orEmpty()
    ...
    private fun isSetValueField(managedEntrySet: Set<Map.Entry<String, Any>>): Boolean =
        isKeyPrefixedSkippingDotKey(managedEntrySet, V_PREFIX)

    private fun isListKeyEntrySet(managedEntrySet: Set<Map.Entry<String, Any>>): Boolean =
        isKeyPrefixedSkippingDotKey(managedEntrySet, K_PREFIX)
